...
const Category = props => {
    ...
    const hasUrlIconRW = data && data.MpRewardIcon ? data.MpRewardIcon.url : null;

    let dataClone = Object.assign({}, data);

    if (dataClone && dataClone.products.items.length) {
        const itemsClone = dataClone.products.items.map((item) => Object.assign({ mp_reward_icon: hasUrlIconRW }, item));
        const productsClone = Object.assign({}, dataClone.products);
        productsClone.items = itemsClone;

        dataClone.products = productsClone;
    }
